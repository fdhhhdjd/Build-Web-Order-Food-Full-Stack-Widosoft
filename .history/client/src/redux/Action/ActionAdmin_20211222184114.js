...
//? upload Img
export const UploadAllStart = () => ({
  type: types.UPLOAD_ALL_USER_ADMIN_START,
});
export const UploadAllSuccess = (token) => ({
  type: types.UPLOAD_ALL_USER_ADMIN_SUCCESS,
  payload: token,
});
export const UploadAllFail = (error) => ({
  type: types.UPLOAD_ALL_USER_ADMIN_FAIL,
  payload: error,
});
//!Register
...
//!Upload IMG
export const UploadImgALInitiate = (token,formData) => {
  return async function (dispatch) {
    dispatch(UploadAllStart());
    await axios
      .post("/cloud/uploadUserImage/admin",formData {
        headers: {
      ...
      .then((user) => {
        dispatch(UploadAllSuccess(user.data));
      })
      .catch((error) => {
        dispatch(UploadAllError(error.data));
      });
  };
};
